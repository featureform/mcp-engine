...
from mcp.server.mcpengine.prompts.base import EmbeddedResource, Message, UserMessage
from mcp.server.mcpengine.resources import FileResource, FunctionResource
...
if TYPE_CHECKING:
    from mcp.server.mcpengine import Context


class TestServer:
    @pytest.mark.anyio
    async def test_create_server(self):
        mcp = MCPEngine(instructions="Server instructions")
        assert mcp.name == "MCPEngine"
        assert mcp.instructions == "Server instructions"

    @pytest.mark.anyio
    async def test_non_ascii_description(self):
        """Test that MCPEngine handles non-ASCII characters in descriptions correctly"""
        mcp = MCPEngine()
    ...
    async def test_resource_multiple_params(self):
        ...
        async with client_session(mcp._mcp_server) as client:
            result = await client.read_resource(
                AnyUrl("resource://cursor/mcpengine/data")
            )
            assert isinstance(result.contents[0], TextResourceContents)
            assert result.contents[0].text == "Data for cursor/mcpengine"
...
class TestServerPrompts:
    """Test prompt functionality in MCPEngine server."""
