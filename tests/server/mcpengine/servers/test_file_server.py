...
def resources(mcp: MCPEngine, test_dir: Path) -> MCPEngine:
...
@pytest.fixture(autouse=True)
def tools(mcp: MCPEngine, test_dir: Path) -> MCPEngine:
    @mcp.tool()
...
@pytest.mark.anyio
async def test_list_resources(mcp: MCPEngine):
...
@pytest.mark.anyio
async def test_read_resource_dir(mcp: MCPEngine):
...
@pytest.mark.anyio
async def test_read_resource_file(mcp: MCPEngine):
...
@pytest.mark.anyio
async def test_delete_file(mcp: MCPEngine, test_dir: Path):
    await mcp.call_tool(
...
@pytest.mark.anyio
async def test_delete_file_and_check_resources(mcp: MCPEngine, test_dir: Path):
    await mcp.call_tool(
        "delete_file", arguments=dict(path=str(test_dir / "example.py"))
    )
    res_iter = await mcp.read_resource("file://test_dir/example.py")
